...
public class AddScoreFrame extends JDialog{
	...
	public AddScoreFrame(JDialog owner, String title, boolean modal,StudentModel sm,int rowNum){
		...
		courses = helper.getCourse(helper.getAllMajor().get(sm.getValueAt(rowNum, 6)),sm.getValueAt(rowNum, 3).toString());//获得所有课目
		...
		//	setBounds(int x,int y,int width,int height);设置组件的x坐标，y坐标，及宽还有高
